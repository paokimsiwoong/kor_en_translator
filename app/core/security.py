...
# @@@ PyJWT 사용으로 변경(Fast API 튜토리얼, 가볍게 JWT만 지원하는 라이브러리, python-jose는 유지보수 거의 중단 됨)
import jwt
from jwt.exceptions import InvalidTokenError

# 비밀번호 해싱 라이브러리
# from passlib.context import CryptContext  # passlib[bcrypt] 라이브러리 설치
# @@@ 최신 pwdlib 사용으로 변경 (passlib는 유지보수 거의 중단 됨)
...
def create_access_token(data: dict, expires_delta: Optional[timedelta] = None) -> str:
    ...
    expire = datetime.now(timezone.utc) + (
        expires_delta or timedelta(minutes=settings.ACCESS_TOKEN_EXPIRE_MINUTES) # expires_delta 가 None이면 False가 되어 or 뒷부분 실행
    )
    # 토큰 만료시점 dict에 추가
...
def decode_access_token(token: str) -> dict:
    ...
    try:
        ...
        # @@@ python-jose와 PyJWT 둘다 코드 동일
        return payload
    except InvalidTokenError: # jwt.decode()는 유효하지 않은 토큰에서 JWTError(python-jose)/InvalidTokenError(PyJWT)를 발생
        raise 
